...
document.addEventListener('DOMContentLoaded', function () {
    ...
    submitBtn.parentElement.appendChild(info);
    ...
    function checkFilled() {
        ...
        if (filled < totalRequired) {
            ...
            info.innerHTML = `⚠️ <span class="text-red-600">${totalRequired - filled} kolom lagi harus diisi.</span>`;
        } else {
            submitBtn.disabled = false;
            info.innerHTML = `✅ <span class="text-green-600">Semua ${totalRequired} kolom sudah diisi. Silakan simpan.</span>`;
        }
    }

    inputs.forEach(input => input.addEventListener('input', checkFilled));
    checkFilled(); // cek saat load pertama
});
</script>
@endpush
